...
Select **Camera server → Local computer** or **External** (enter its IP), then **Update fields from camera**. The popup receives labels through the supplied Scanner SDK; only Quantity is editable. Enter prints and keeps scanning, Space copies the fields to the main window, and Escape leaves. See [camera setup and behavior](docs/camera.md).
...
Use the separate **Use '00'** checkboxes below Month and Year when either date segment is unknown. Only the checked selector is disabled; its segment becomes 00 in the complete lot. L3 encodes version, raw PN, lot and padded quantity in one Data Matrix with a CRC-32 checksum. Quantity is always printed. See [lot and barcode rules](docs/lot-format.md).
